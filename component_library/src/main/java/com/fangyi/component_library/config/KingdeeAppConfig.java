...
import com.fangyi.component_library.func.utils.FileUtils;
 ...
 */
public class KingdeeAppConfig {

    private static KingdeeAppConfig sInstance;

    public static KingdeeAppConfig get() {
        if (sInstance == null)
            synchronized (KingdeeAppConfig.class) {
                if (sInstance == null)
                    sInstance = new KingdeeAppConfig();
            }
        return sInstance;
    }
    ...
    private KingdeeAppConfig() {
        this.mPreferences = Utils.getApp().getSharedPreferences("Kalle", Context.MODE_PRIVATE);

        this.PATH_APP_ROOT = FileUtils.getAppRootPath(Utils.getApp()).getAbsolutePath() + File.separator + "Kalle";
